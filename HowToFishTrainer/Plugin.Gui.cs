...
namespace HowToFishTrainer
{
    public partial class Plugin
    {
        ...
        // Controls for features that write server state; a client in an agreed
        // room sees them greyed out.
        private bool _guiEnabledBeforeHostOnly;
        private string HostTag => _isHost ? "" : " [仅房主]";

        private void BeginHostOnly()
        {
            _guiEnabledBeforeHostOnly = GUI.enabled;
            GUI.enabled = GUI.enabled && _isHost;
        }

        private void EndHostOnly()
        {
            GUI.enabled = _guiEnabledBeforeHostOnly;
        }
        ...
        {
            GUILayout.Space(8);
            GUILayout.Label(M("金钱 (启用后也可用游戏自带的 M / N 键)" + HostTag));
            BeginHostOnly();
            GUILayout.BeginHorizontal();
            ...
            {
                MoneyManager.AddMoney(99999, Player.LocalPlayer);
            }
            if (GUILayout.Button(M("解锁全部皮肤")))
            {
                DazedCommands.IsServerCommand("/allskins");
            }
            GUILayout.EndHorizontal();
            EndHostOnly();
        }
        ...
        private void DrawCombatSection()
        {
            ...
            IgnoreBossImmortality = GUILayout.Toggle(IgnoreBossImmortality,
                M("无视 Boss 无敌 (Boss 播动画/换阶段/多只同时存在时也能打)" + HostTag));
            EndHostOnly();
            ...
            BeginHostOnly();
            LockHealth = GUILayout.Toggle(LockHealth, M("锁血 (免疫伤害/中毒/着火)" + HostTag));
            _lockFullness = GUILayout.Toggle(_lockFullness, M("锁饱食度" + HostTag));
            EndHostOnly();
        ...
        {
            GUILayout.Space(8);
            GUILayout.Label(M("赌场 - 轮盘 (押鱼/物品，黑红 2 倍、绿 35 倍):" + HostTag));
            BeginHostOnly();
            ...
            foreach (var item in _skinItems)
            {
                ...
                {
                    SlotMachineManager.SetCheatSkin(item, item.SkinPreset.GetRandomSkinIndex(Rarity.Legendary));
                }
            }
            GUILayout.EndScrollView();
        ...
        {
            GUILayout.Space(8);
            GUILayout.Label(M("生成鱼类/生物 (在准星前方):" + HostTag));
            BeginHostOnly();
            EnsureSpawnList();
            ...
            foreach (var entry in _spawnables)
            {
                ...
                {
                    DazedCommands.IsServerCommand("/spawn " + entry.key);
                }
            }
            GUILayout.EndScrollView();
            EndHostOnly();
